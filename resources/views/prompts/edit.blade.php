...
<head>
    ...
    <title>Edit {{ $prompt->title }} - Prompt Library</title>
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="mb-8">
            <div class="flex items-center space-x-2 text-sm text-gray-500 mb-4">
                ...
                <span>/</span>
                <a href="{{ route('prompts.show', $prompt) }}" class="hover:text-gray-700">{{ $prompt->name }}</a>
                <span>/</span>
                <span>Edit</span>
            </div>
            <h1 class="text-3xl font-bold text-gray-900">Edit {{ $prompt->title }}</h1>
        </div>

        <form method="POST" action="{{ route('prompts.update', $prompt) }}" class="bg-white rounded-lg shadow-sm border p-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title', $prompt->title) }}" required
                ...
                <div>
                    ...
                    <select name="category" id="category" required
                        ...
                        @foreach($categories as $category)
                            <option value="{{ $category }}" {{ old('category', $prompt->category) === $category ? 'selected' : '' }}>
                                {{ ucfirst($category) }}
                            </option>
                        @endforeach
                ...
                <div>
                    ...
                    <input type="text" name="tags" id="tags" value="{{ old('tags', implode(', ', $prompt->tags ?? [])) }}"
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" id="description" rows="3"
                              ...
                              placeholder="Briefly describe what this prompt does...">{{ old('description', $prompt->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="content" id="content" rows="12" required
                              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 font-mono text-sm @error('content') border-red-500 @enderror">{{ old('content', $prompt->content) }}</textarea>
                    @error('content')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="md:col-span-2">
                    <div class="flex items-center">
                        <input type="checkbox" name="is_public" id="is_public" value="1"
                               {{ old('is_public', $prompt->is_public) ? 'checked' : '' }}
